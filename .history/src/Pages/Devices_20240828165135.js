...
const Devices = () => {
  ...
  const fetchHandler = () => {
    getApi(endPoints.devices.getDevices({status}), {
  ...
  const tbody = devices.data.docs.map((i) => [
    ...
    i?.status === "Disconnected" ? (
      <div className="w-[139px] h-[34px] bg-[#EDF8F0] rounded-xl text-[#939eb9] flex justify-center gap-1 items-center m-auto" style={{fontWeight : '900'}} >
        Disconnected
      </div>
    ) : (
      <div
